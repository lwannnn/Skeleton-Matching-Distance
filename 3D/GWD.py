...
class GraphWassersteinDistance(nn.Module):
    ...
    def get_dist(self,mu,nu,C,A,mask=None):
        """
        :param mu: the Distribution of Predicted graph
        :param nu: the Distribution of GT
        :param C: cost metrix
        :param A: mask matrix（knn）
        :param mask
        :return:
        """
...
def extract_centerlines(segmentation):
    ...
    :param segmentation: binary 3D numpy
    :return: binary 3D numpy
    """
...
#     pre = extract_centerlines(Prediction_3D_numpy)#convert to 3dnumpy and then extract skeletons
#     GT = extract_centerlines(GT_3D_numpy)
#     gwd = loss(pre.cuda(),GT.cuda())
#     print(gwd)
